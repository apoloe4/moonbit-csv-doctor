...
可选空值跳过类型、范围、枚举、唯一性检查。boolean 仅接受小写 `true`、`false`。number 按 JSON 数值语法解析为 IEEE-754 Double，不接受 NaN/Infinity；不适用于高精度财务计算。integer 限制绝对值不超过 9007199254740991。唯一性比较原始字符串，因此 `1` 与 `1.0` 不视为重复。
...
开发使用 AI 辅助编写和检查代码。参赛者需理解状态机、规则语义和测试结果，并自行提交赛事报名；本仓库的创建不表示已报名或通过验收。
